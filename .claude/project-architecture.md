...
前端生产 CI/CD workflow。Push 到 `main` 或手动触发时，先运行 lint、test 和 production build；验证通过后使用 GitHub Actions Secrets 中的 SSH 凭证把 `dist/` 打包上传到服务器 `/var/www/dev-time/releases/{sha}`，并更新 `/var/www/dev-time/current` 软链接。
...
| 2026-06-16 | 增加前端 GitHub Actions 生产部署 workflow | 1.0 需要通过 CI/CD 构建并发布静态前端到服务器 | `ruby -e 'require "yaml"; YAML.load_file(".github/workflows/deploy.yml")'` |

...
`.claude` 目录保存 `dev-time` 的长期上下文，供人类维护者和 Agent 在开发、排障、评审时快速理解项目约定。
...
- `demo-design-references.md`：HTML demo 的 Dribbble 参考来源、可借鉴点和落地设计决策。
...
- `coding-standards.md`：前端编码规范、文件职责、行数约束和评审检查项。
...
- 调整 `demo/index.html` 的视觉方向或交互结构时，同步更新 `demo-design-references.md`。
